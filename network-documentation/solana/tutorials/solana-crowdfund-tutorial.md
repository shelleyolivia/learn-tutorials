...
And here comes the `mut` keyword. We can intitalize variable with `let mut` to have mutable variable. eg.
```rust
...
// and how we can have a i32 value stored in `Custom`
...
// We can use the match syntax to see, what type of coin we have  
...
We are using it for data serialization and deserilization.
\
\
\
...
3. We will discuss the frontend side of the project later. Now we can open the `program` folder in vs-code. 
```bash
code program/
```
...
We would need an entry point with which anyone can use to create a crowd funding campaign on our platform. We can have a name, description and an admin fields for it.

- #### Withdraw from the Campaign.
We would need an entry point for campaign admin's only, so they can withdraw funds.

- #### Donate to an Campaign.
We would need an entry point which can be invoked by anyone to donate to a specific Campaign.
...
> Note: The total number of bytes of data a account can have is fixed when we create an account.

One way to deal with this is to create a Account with very large storage. But If we do that the max data limit for an account is 10 mb. If we have so many users that we will run out of storage space. We must think of a way to have infinite amount of storage.
...
> Note: In rust we can pass variables as reference with `&`.
...
// Then we call the entrypoint macro to add `process_instruction` as our entrypoint to our program.
...
But we want three as, we discussed in `What we want in our program?` section.
Lets overcome this issue.
Have you noticed there is no limit to the instruction_data array. We are going to take advantage of that fact.We are going to use the first element of the array to know what entrypoint we want to call.
Notice we can have 256 entrypoints like this in a single program (`u8` has a value of 0..255). Realistically we never do that if we want that many entrypoints for a project it is better to deploy more number of programs.

Okay, lets do more coding coding...
...
/// Here I have create the the function for every action we want to do in our program.
...
```
Now you must be wondering what is the the meaning of `#[derive(Debug)]`. It is interesting to note that we can derive some traits for our struct.
...
Okay, Now lets see what is our `CampaignDetails` struct.
...
```
 We can use the `next_account_info` function to get a account from the array. This function returns a result enum. We can use `?` Operator on result enum to get the value. If in case of error the `?` Operator will chain the error and our program will return the same error which was returned by next_account_info.
...
```rust   
    ...
    /// This is an account we will create in our frontend.
...
```
By deriving the trait `BorshDeserialize` in our `CampaignDetails` struct we have added a method `try_from_slice` which take in the parameter `array of u8` and creates a object of CampaignDetails with it.
...
```
Solana account's can have a data, but size has to be specified when it is created and also we need to have a minimum balance to make it rent exempt. For this project we create account which already has a balance equal to minimum balance.
...
```
If all goes well we will write the writing_account. Here on our `input_data`(type `CampaignDetails`) variable we  have a method `serialize` this is because of the `BorshSerialize` derivation. We will use this and write the data in writing account. At the end of the program we can return `Ok(())`.
```rust
...
So for this we will create program owned account in our front-end and then make the sol-token transaction.
...
Then we have account of the `donator`.
> Note: When we will create it in our frontend, although we do not want to store anything in it we will assign it some size. So that it automatically gets deleted after the sol token the transferred.
...
Now we are going to request an airdrop for sol token on `devnet`.
This command will add sol tokens to account
```bash
...
This is not a react tutorial, so I will not go into the details of react.
...
If you want the UI part only and continue coding the solana(web.js)  libaray you can use the ui template I created for you from [github ui branch](https://github.com/SushantChandla/solana-crowd-funding/tree/ui).
...
The first thing we will do it to add a call to checkWallet, which would connect the wallet if it is not connect.
```js
...
```
Here we create the instruction to create our account we will pass in the pub key, the size of data it need to store, initial lamports and other parameters.
...
Then We will pass 3 keys(Accounts) as our program need 3 accounts(see `donate` function implementation in program).
...
```
We have created instruction to our program and then called the `setPayerAndBlockhashTransaction`, `signAndSendTransaction` and `confirmTransaction` to send and confirm the transaction just like we did for the `createCampaign` function.
